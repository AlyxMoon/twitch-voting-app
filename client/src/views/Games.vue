  ...
  <div>
    ...
    <div class="table-wrapper">
      ...
      <table class="pure-table pure-table-horizontal full-width">
        ...
        <tbody>
          <tr v-for="game in banned" :key="'game-' + game.id">
            <td>{{ game.name }}</td>
            <td>{{ game.guid }}</td>
          </tr>
        </tbody>
    ...
    <div class="table-wrapper">
      ...
      <table class="pure-table pure-table-horizontal full-width">
        ...
        <tbody>
          <tr v-for="game in cached" :key="'game-' + game.id">
            <td>{{ game.name }}</td>
            <td>{{ game.guid }}</td>
          </tr>
        </tbody>
  ...
  beforeRouteEnter (to, from, next) {
    fetchJSON(`${serverAddress}/api/games`)
      .then(result => {
        ...
        return next(vm => vm.setData({ cached: result.data }))
      })
      .catch(error => next(vm => vm.setData({ error: error })))
  },

  methods: {
    setData ({ error, aliases, banned, cached }) {
      if (error) {
        console.error(error)
        this.error = error
      }
